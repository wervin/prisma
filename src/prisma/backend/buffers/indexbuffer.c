    ...
    struct prisma_backend_buffers_indexbuffer_info *info)
{
    ...
    void* data;
    vkMapMemory(device->vk_device, staging_buffer.vk_devicememory, 0, info->index_count * info->index_size, 0, &data);
    memcpy(data, info->data, info->index_count * info->index_size);
    vkUnmapMemory(device->vk_device, staging_buffer.vk_devicememory);

    struct prisma_backend_buffer_info indexbuffer_info = {
        .size = info->index_count * info->index_size,
        .memory_properties = VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT,
        .usage = VK_BUFFER_USAGE_TRANSFER_DST_BIT | VK_BUFFER_USAGE_INDEX_BUFFER_BIT
    };

    status = prisma_backend_buffer_create(&indexbuffer->buffer, device, &indexbuffer_info);
    if (status != PRISMA_ERROR_NONE)
        return status;

    status = prisma_backend_buffer_copy(&staging_buffer, &indexbuffer->buffer, info->index_count * info->index_size, device, commandpool);
    if (status != PRISMA_ERROR_NONE)
        return status;
...
{
    prisma_backend_buffer_destroy(&indexbuffer->buffer, device);
}